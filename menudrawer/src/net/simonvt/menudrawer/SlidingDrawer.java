...
public class SlidingDrawer extends DraggableDrawer {
        ...
        switch (getPosition()) {
            ...
            case BOTTOM: {
                if (mIsDragging) {
                    ...
                    {
                    case None:
                    	Log.d(TAG, String.format("None"));
                    	menuState = MenuState.FirstLevel;
                    	break;
                    case FirstLevel:
                    	Log.d(TAG, String.format("First"));
                    	y = y*menuHeightRatio;
                    	mMenuSize = mMenuSize*menuHeightRatio;
                    	menuState = MenuState.SecondLevel;
                    	break;
					case SecondLevel:
						Log.d(TAG, String.format("second"));
						break;
					default:
						break;                    
                    }
    ...
    public void closeMenu() {
		super.closeMenu();
		 if (menuState == MenuState.SecondLevel) {
			 mLastMotionY = mLastMotionY/menuHeightRatio;
         	mMenuSize = mMenuSize/menuHeightRatio;
		}                   
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        ...
        if (mVelocityTracker == null) mVelocityTracker = VelocityTracker.obtain();
        mVelocityTracker.addMovement(ev);
        Log.d(TAG, String.format("%s", action));
        switch (action) {
            case MotionEvent.ACTION_DOWN: {
            ...
            case MotionEvent.ACTION_MOVE: {
            	Log.d(TAG, String.format("ACTION_MOVE %d:", ev.getAction()));
                ...
                if (mIsDragging) {
                	Log.d(TAG, String.format("ACTION_MOVE Dragiing %d:", ev.getAction()));
                    startLayerTranslation();
                    ...
                    mLastMotionY = y;
                    onMoveEvent(dx, dy);
                    Log.d(TAG, String.format("moving %s", action));
                }
                break;
            }
